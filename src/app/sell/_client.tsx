      ...
      <div className="mt-12 flex items-center gap-3">
        {STEPS.map((label, i) => (
          <div key={label} className="flex items-center gap-3 flex-1">
            <span className={`h-2 w-2 rounded-full ${i <= step ? "bg-foreground" : "bg-border"}`} />
            <span
              className={`text-xs tracking-wider uppercase ${i === step ? "text-foreground" : "text-muted-foreground"}`}
            >
              {label}
            </span>
